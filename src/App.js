...
class App extends Component {
  ...
  imageClick = thumbnailUrl => {
    ...
    const imgValue = imagesList.filter(
      eachImg => eachImg.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = imgValue[0]
    if (imageUrl === imgUrl) {
      const newImgUrl =
  ...
  render() {
    const {score, timer, gameOver, imgUrl, category} = this.state
    const thumbnailList = imagesList.filter(each => each.category === category)
    return (
        ...
        <div className="bottom-container">
          {!gameOver && (
            <div className="flex-bottom">
                  ...
                  <li key={eachValue.tabId}>
                    <button
                      ...
                      className={`tab-button ${
                        category === eachValue.tabId ? 'highlighted-text' : ''
                      }`}
              ...
              <ul className="thumbnailList-container">
          ...
          {gameOver && (
            <div className="game-over-img">
              <img
                ...
                className="game-over-trophy"
                alt="trophy"
              />
              <p className="game-text">YOUR SCORE</p>
